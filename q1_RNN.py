...
class RNN:
    ...
    def forward(self, X, a_prev):
        ...
        for i in range(len(self.X)):
            ...
            a[i] = np.tanh(np.dot(self.Waa, a[i-1]) + np.dot(self.Wax, x[i]) + self.ba)
    ...
    def loss(self, y_preds, targets):
        ...
        # loss = 0
        # loss = sum(-np.log(y_preds[t][targets[t], 0]) for t in range(len(self.X)))
        loss = float(loss)
        ### STUDENT CODE ENDS HERE
    ...
    def predict(self, start):
        ...
        while (idx != newline_character and counter != max_chars):
            ...
            a = np.tanh(np.dot(self.Wax, x) + np.dot(self.Waa, a_prev) + self.ba)
            # Update input vector, previous hidden state, and indices and counter
