...
class ConvolutionLayer:
        ...
        for f in range(self.num_filters):
            for i in range(height-self.filter_size+1):
                for j in range(width-self.filter_size+1):
                    Xs = self.input[i:i+F,j:j+F]
                    Ks = self.conv_filter[f]
                    output[f,i,j] = np.sum(np.sum(Xs*Ks, axis = 1),axis = 0) + self.bias[f]
        ## END TODO
    ...
    def backward(self, output_error, learning_rate):
        ...
        for f in range(self.num_filters):
            ...
            for i in range(height - self.filter_size + 1):
                for j in range(width - self.filter_size + 1):
                    input_error[i:i+self.filter_size, j:j+self.filter_size] += output_error[f, i, j] * self.conv_filter[f]
                    filter_error[f] += output_error[f, i, j] * self.input[i:i+self.filter_size, j:j+self.filter_size]
        ...
        for f in range(self.num_filters):
            for i in range(self.filter_size):
                for j in range(self.filter_size):
                    self.conv_filter[f, i, j] -= learning_rate * filter_error[f, i, j]
        ## END TODO
...
class ReLULayer:
    ...
    def backward(self, output_error, learning_rate):
        ...
        input_error = None
        ## TODO
        one_S = np.sign(np.maximum(0,self.input))
        input_error = output_error*one_S
        ## END TODO
...
class MaxPoolingLayer:
    ...
    def forward(self, input):
        ...
        ## TODO
        F = self.filter_size
        output = np.zeros((num_filters,height//F,width//F))
        for f in range(num_filters):
            for i in range(height//F):
                for j in range(width//F):
                    output[f,i,j] = np.max(input[f, i*F:(i+1)*F , j*F:(j+1)*F])
    ...
    def backward(self, output_error, learning_rate):
        ...
        num_filters, pool_height, pool_width = output_error.shape
        for f in range(num_filters):
            for i in range(pool_height):
                for j in range(pool_width):
                    # Get the position of the maximum value in the corresponding input region
                    max_index = np.argmax(self.input[f, i*self.filter_size:(i+1)*self.filter_size, j*self.filter_size:(j+1)*self.filter_size])
                    # Convert the index to the original input coordinates
                    max_i, max_j = np.unravel_index(max_index, (self.filter_size, self.filter_size))
                    # Assign the error to the position of the maximum value
                    input_error[f, i*self.filter_size+max_i, j*self.filter_size+max_j] = output_error[f, i, j]
        ## END TODO
...
class FlattenLayer:
    ...
    def backward(self, output_error, learning_rate):
        ...
        ## TODO
        num_filters,height,width = self.input_shape
        input_error = output_error.reshape(num_filters,height,width)
        ## END TODO
